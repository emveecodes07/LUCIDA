...
# Ollama defaults to a tiny 2048-token context window regardless of what the
# underlying model actually supports. That silently truncates older turns —
# the model "forgets" earlier parts of the conversation even though history
# was sent — which is the #1 cause of replies that seem to lose the thread.
# Explicitly requesting a larger window fixes that. Passed on every call
# (including warm_up) so the model is never reloaded mid-conversation just
# because the context size changed between requests.
...
def warm_up(base_url: str, model: str, keep_alive: str = DEFAULT_KEEP_ALIVE,
    ...
    try:
        client.chat.completions.create(
            ...
            max_tokens=1,
            temperature=0,
            extra_body={"keep_alive": keep_alive, "num_ctx": num_ctx},
        )
    except Exception:
        pass
...
def complete(base_url: str, model: str, system_prompt: str, user_content: str,
             ...
             keep_alive: str = DEFAULT_KEEP_ALIVE, num_ctx: int = DEFAULT_NUM_CTX) -> str:
