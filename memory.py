...
DEFAULT_MEMORY = {
    ...
    "calendar_events": [],  # list of {"id", "date": "YYYY-MM-DD", "title", "note", "pinned": bool}
}
...
def load_memory(path: str) -> dict:
    ...
    try:
        ...
        merged["calendar_events"] = (
            data.get("calendar_events", []) if isinstance(data.get("calendar_events"), list) else []
        )
        return merged
...
# ---------------------------------------------------------------------------
# Calendar — a small, user-editable event list backing the always-available
# Calendar tab. Lives in the same JSON memory file as everything else, so
# events are "active memory": they persist across restarts and across
# persona switches without any extra plumbing. Kept dead simple (flat list,
# no recurrence rules) since this is a personal local tool, not a real
# calendar app. Each event gets a short uuid id rather than a list index, so
# deleting/pinning one event never shifts another event's identity out from
# under an in-flight UI action.
# ---------------------------------------------------------------------------

def add_event(memory: dict, entry_date: str, title: str, note: str = "", pinned: bool = False) -> dict:
    memory.setdefault("calendar_events", []).append({
        "id": uuid.uuid4().hex[:8],
        "date": entry_date,
        "title": title.strip(),
        "note": note.strip(),
...
def update_event(memory: dict, event_id: str, **fields) -> dict:
    for e in memory.get("calendar_events", []):
        if e["id"] == event_id:
            for k in ("date", "title", "note", "pinned"):
                if k in fields:
                    e[k] = fields[k]
            break
    return memory
...
def delete_event(memory: dict, event_id: str) -> dict:
    memory["calendar_events"] = [e for e in memory.get("calendar_events", []) if e["id"] != event_id]
    return memory


def toggle_pin(memory: dict, event_id: str) -> dict:
    for e in memory.get("calendar_events", []):
        if e["id"] == event_id:
...
def calendar_events_sorted(memory: dict) -> list:
    return sorted(memory.get("calendar_events", []), key=lambda e: e.get("date", ""))


def pinned_events(memory: dict) -> list:
    return [e for e in calendar_events_sorted(memory) if e.get("pinned")]
