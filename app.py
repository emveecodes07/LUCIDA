...
HISTORY_TRIM_AT = 28        # rewrite older turns into a rolling summary past this many messages
KEEP_RECENT_MESSAGES = 12   # how many raw turns stay verbatim after a summary rollup — higher than
                             # before so replies actually track what was just said instead of
                             # losing the thread a few turns back (paired with a bigger Ollama
                             # context window in llm_client.DEFAULT_NUM_CTX)
...
st.markdown(
    ...
    <style>
        ...
        .mode-sil        { background: linear-gradient(135deg, #d1b28c, #9c7b4e); color: #2e1f0a; }
        .mode-mrin       { background: linear-gradient(135deg, #ff6bcb, #7c3aed); color: #ffffff; }
        .mode-bbd        { background: linear-gradient(135deg, #ffb3ba, #ff6f91); color: #5c0e26; }
        .mode-goat       { background: linear-gradient(135deg, #ffe066, #f4a300); color: #4a2e00; }
...
full_form = resolved_persona.get("full_form")
if full_form:
    header_col.caption(f"*{full_form}*")


# ----------------------------------------------------------------------------
# Helper: build the outgoing message list (system prompt + memory + history)
# ----------------------------------------------------------------------------
# Appended to every persona, on every call — a shared, model-facing reminder
# to actually track the conversation and hold its own voice for the whole
# reply rather than drifting into something generic a few turns in. Kept
# separate from the per-persona prompts so every mode gets it consistently.
GROUNDING_RULE = (
    "\n\nCONVERSATION GROUNDING: Read the actual messages below before replying — respond to "
    "what the user just said, specifically, not a generic version of the topic. If their last "
    "message is ambiguous or you're missing something you'd need to answer well, ask ONE short, "
    "specific question instead of guessing. Stay fully in character and keep your voice and hard "
    "rules above consistent for this entire reply, not just the opening line."
)
...
def render_chat_panel(doc_context: str) -> None:
    ...
    # Zero-model-call nudge: if this message reads like it mentions a real
    # day/date worth remembering (an exam, a due date, an appointment...),
    # offer a one-tap "pin it" popup. Pure regex, same cost profile as the
    # persona-switch suggestion below — it never touches what gets sent to
    # the model and never delays or alters the persona's actual reply.
    if cal.is_pin_worthy(user_prompt):
        pin_row = st.container()
        with pin_row:
            msg_col, date_col, btn_col = st.columns([4, 2, 2])
            with msg_col:
                st.info("📅 Sounds like there's a date in here — want to pin it to your calendar?")
            with date_col:
                pin_date = st.date_input(
                    "Date", value=date.fromisoformat(cal.guess_date(user_prompt)),
                    key=f"pin_date_{len(history)}", label_visibility="collapsed",
                )
            with btn_col:
                if st.button("📌 Pin it", key=f"pin_btn_{len(history)}", use_container_width=True):
                    st.session_state.memory = mem.add_event(
                        st.session_state.memory, pin_date.isoformat(),
                        title=user_prompt[:80],
                        note=f"From {PERSONAS[active_mode]['sidebar_label']} chat",
                        pinned=True,
                    )
                    mem.save_memory(MEMORY_PATH, st.session_state.memory)
                    st.toast("Pinned to your calendar!", icon="📌")
...
def _render_event_row(e: dict) -> None:
    row1, row2, row3, row4 = st.columns([2, 4, 1, 1])
    row1.caption(e["date"])
    label = f"**{e['title']}**" + (f" — {e['note']}" if e.get("note") else "")
    row2.markdown(label)
    if row3.button("📌" if e.get("pinned") else "📍", key=f"pin_toggle_{e['id']}",
                    help="Pinned — click to unpin" if e.get("pinned") else "Click to pin"):
        st.session_state.memory = mem.toggle_pin(st.session_state.memory, e["id"])
        mem.save_memory(MEMORY_PATH, st.session_state.memory)
        st.rerun()
    if row4.button("🗑️", key=f"del_event_{e['id']}", help="Delete"):
        st.session_state.memory = mem.delete_event(st.session_state.memory, e["id"])
        mem.save_memory(MEMORY_PATH, st.session_state.memory)
        st.rerun()


def render_month_grid(memory: dict) -> None:
    """A real, physical-looking month calendar — 7-day grid, one row per
    week, every event shown as a little chip on its actual date. Purely a
    view over the same calendar_events list the form/list below edit, so
    anything added, pinned, or deleted anywhere shows up here too."""
    ...
    if nav_prev.button("◀", key="cal_prev_month", use_container_width=True):
        m, y = st.session_state.cal_view_month - 1, st.session_state.cal_view_year
        if m < 1:
            m, y = 12, y - 1
        st.session_state.cal_view_month, st.session_state.cal_view_year = m, y
        st.rerun()
    nav_label.markdown(
        f"<div style='text-align:center;font-weight:700;padding-top:4px;'>"
        f"{pycal.month_name[st.session_state.cal_view_month]} {st.session_state.cal_view_year}"
        f"</div>",
        unsafe_allow_html=True,
    )
    if nav_today.button("Today", key="cal_today_month", use_container_width=True):
        st.session_state.cal_view_year = today.year
        st.session_state.cal_view_month = today.month
        st.rerun()
    if nav_next.button("▶", key="cal_next_month", use_container_width=True):
        m, y = st.session_state.cal_view_month + 1, st.session_state.cal_view_year
        if m > 12:
            m, y = 1, y + 1
        st.session_state.cal_view_month, st.session_state.cal_view_year = m, y
    ...
    year, month = st.session_state.cal_view_year, st.session_state.cal_view_month

    events_by_day: dict = {}
    for e in memory.get("calendar_events", []):
        try:
    ...
    weeks = pycal.Calendar(firstweekday=0).monthdayscalendar(year, month)  # Monday-first

    header_cols = st.columns(7)
    for h, wd in zip(header_cols, ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]):
        h.markdown(
            f"<div style='text-align:center;font-size:0.75em;opacity:0.6;"
            f"font-weight:700;letter-spacing:0.04em;'>{wd}</div>",
            unsafe_allow_html=True,
        )
    ...
    for week in weeks:
        ...
        for col, day in zip(cols, week):
            with col:
                if day == 0:
                    st.markdown(
                        "<div style='min-height:78px;border-radius:8px;"
                        "background:rgba(255,255,255,0.02);'></div>",
                        unsafe_allow_html=True,
                    )
                    continue
                is_today = day == today.day and month == today.month and year == today.year
                day_events = sorted(events_by_day.get(day, []), key=lambda e: not e.get("pinned"))
                cell_bg = "rgba(124,157,255,0.14)" if is_today else "rgba(255,255,255,0.035)"
                border = "2px solid #7c9dff" if is_today else "1px solid rgba(255,255,255,0.09)"
                chips = ""
                for e in day_events[:3]:
                    pin = "📌 " if e.get("pinned") else ""
                    title = e["title"][:13] + ("…" if len(e["title"]) > 13 else "")
                    chips += (
                        "<div style='font-size:0.66em;background:rgba(124,157,255,0.22);"
                        "border-radius:4px;padding:1px 4px;margin-top:2px;overflow:hidden;"
                        f"white-space:nowrap;text-overflow:ellipsis;' title='{e['title']}'>"
                        f"{pin}{title}</div>"
                    )
                if len(day_events) > 3:
                    chips += (
                        f"<div style='font-size:0.6em;opacity:0.65;margin-top:1px;'>"
                        f"+{len(day_events) - 3} more</div>"
                    )
                st.markdown(
                    f"<div style='min-height:78px;border:{border};border-radius:8px;"
                    f"background:{cell_bg};padding:4px;'>"
                    f"<div style='font-size:0.78em;font-weight:{'800' if is_today else '500'};'>"
                    f"{day}</div>{chips}</div>",
                    unsafe_allow_html=True,
                )
    st.caption("📌 pinned entries sort first in each day. Full details for every entry are below.")


def render_calendar_tab() -> None:
    """User-editable calendar with pinning, stored in the same local
    lucida_memory.json as everything else — so it's active memory that
    persists across restarts and is shared across every persona's chat,
    not scoped to whichever mode you happened to add it from."""
    st.subheader("🗓️ Add an event")
    with st.form(key=f"cal_form_{st.session_state.cal_form_nonce}", clear_on_submit=True):
        c1, c2 = st.columns(2)
        ev_date = c1.date_input("Date", value=date.today())
        ev_title = c2.text_input("What is it?", placeholder="e.g. DBMS mid-sem, rent due, mom's birthday")
        ev_note = st.text_input("Note (optional)", placeholder="any extra detail")
        ev_pin = st.checkbox("📌 Pin this — shows at the top")
        ...
        if submitted:
            if not ev_title.strip():
                ...
                st.session_state.memory = mem.add_event(
                    st.session_state.memory, ev_date.isoformat(), ev_title, ev_note, ev_pin,
                )
                mem.save_memory(MEMORY_PATH, st.session_state.memory)
                st.session_state.cal_form_nonce += 1
                st.toast("Added to your calendar!", icon="🗓️")
                st.rerun()

    st.divider()
    render_month_grid(st.session_state.memory)

    st.divider()
    pinned = mem.pinned_events(st.session_state.memory)
    if pinned:
        st.subheader("📌 Pinned")
        for e in pinned:
            _render_event_row(e)
    ...
    all_events = mem.calendar_events_sorted(st.session_state.memory)
    today_iso = date.today().isoformat()
    upcoming = [e for e in all_events if e["date"] >= today_iso]
    past = [e for e in all_events if e["date"] < today_iso]

    if not all_events:
        st.caption("Nothing on the calendar yet — add something above, or pin a date mentioned in chat.")
        return
    ...
    if upcoming:
        ...
        st.caption("Nothing upcoming — everything logged so far is in the past.")
    if past:
        with st.expander(f"Past ({len(past)})"):
            for e in past:
                _render_event_row(e)
...
# gets a Spending Tracker tab. Every persona gets a Calendar tab — it's
# global, shared, user-editable state, not scoped to any one mode's work.
# ----------------------------------------------------------------------------
show_doc_hub = active_mode in ("Study Mode", "Deep Focus Mode")
show_finance_hub = active_mode == "Financial Advisor"

if show_doc_hub:
    tab_chat, tab_pack, tab_cal = st.tabs(["💬 Chat", "📚 Study Pack", "📅 Calendar"])
    doc_context = (
    ...
    tab_chat, tab_tracker, tab_cal = st.tabs(["💬 Chat", "💰 Spending Tracker", "📅 Calendar"])
    with tab_tracker:
    ...
    tab_chat, tab_cal = st.tabs(["💬 Chat", "📅 Calendar"])
    with tab_chat:
        render_chat_panel("")
    with tab_cal:
        render_calendar_tab()
